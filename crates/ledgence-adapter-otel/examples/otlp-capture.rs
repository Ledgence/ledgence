...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    eprintln!(
        "OTLP_CAPTURE_METRICS_ENDPOINT=http://{}/v1/metrics",
        listener.local_addr()?
    );
    ...
    for connection in listener.incoming() {
        ...
        match receive(&mut connection) {
            Ok(request) => {
                match request {
                    CaptureRequest::Metrics(request) => metrics::emit(request, &mut output)?,
                    CaptureRequest::Traces(request) => {
...
fn receive(stream: &mut TcpStream) -> Result<CaptureRequest, Box<dyn std::error::Error>> {
    stream.set_read_timeout(Some(Duration::from_secs(5)))?;
    stream.set_write_timeout(Some(Duration::from_secs(5)))?;
    let mut reader = BufReader::new(stream);
    let mut line = String::new();
    reader.by_ref().take(8193).read_line(&mut line)?;
    let path = match line.as_str() {
        "POST /v1/traces HTTP/1.1\r\n" => "/v1/traces",
        "POST /v1/metrics HTTP/1.1\r\n" => "/v1/metrics",
        _ => return Err("expected POST /v1/traces or /v1/metrics".into()),
    };
    let mut length = None;
    ...
    if path == "/v1/metrics" {
        Ok(CaptureRequest::Metrics(
            ExportMetricsServiceRequest::decode(body.as_slice())?,
        ))
    } else {
        Ok(CaptureRequest::Traces(ExportTraceServiceRequest::decode(
            body.as_slice(),
        )?))
    }
}
