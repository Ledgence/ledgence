...
The worker first treats an assignment as awaiting authority. No package preparation or execution begins until an immediate Dispatch renewal establishes its first local bound. The renewal request-start time excludes the earlier acquisition hold. Initial confirmation has a fixed 30-second budget; an unresolved or stopped attempt cannot later regain local permission. The worker retains request-start monotonic accounting: connection, transfer, and decoding time consume authority. Assignment delivery still requires a dispatch renewal before package preparation/execution. Late responses cannot revive locally stopped work. One `--concurrency N` (default `4`, range `1–1024`) controls N consumers and at most N managed subprocesses globally within that worker. Multiple workers each have their own N and service session; N is not a cluster-wide limit.
...
Submission `origin_trace`, invocation event trace context, and settlement `processing_trace` retain separate meanings. With tracing enabled, every HTTP call gets its own client/server spans and W3C version-00 exchange carrier. Transport headers never overwrite the JSON command's origin or processing context. Duplicate or malformed `traceparent` fields are ignored. Multiple `tracestate` fields are combined in order; invalid or oversized state is discarded while preserving a valid parent, following the [W3C trace-context rules](https://www.w3.org/TR/trace-context/#tracestate-header). Invalid tracing metadata never rejects an otherwise valid command; diagnostics are bounded and do not echo header values.

A newly allocated invocation receives a producer span parented to the accepted submission origin, with the acquisition exchange linked separately. The producer context is persisted in the event before commit. Response replay returns that exact event without recreating a historical producer. Retries create sibling producer spans under the original submission origin; originless attempts start independent traces joined by their durable task/run IDs. If tracing is disabled, newly generated events keep the existing submission origin or its absence. The worker captures its processing span once for settlement, including retries. Export loss or sampling never changes durable delivery outcomes.
...
The first SIGINT/SIGTERM makes the orchestrator unready, closes new request admission, and drains accepted HTTP operations. Accepted acquisition waits wake into their final atomic probe; they may claim work or replay a committed result. Recovery continues during HTTP drain, then stops between bounded batches. Optional notification tasks and their auxiliary pool close before the lifecycle database pool. Observation timeouts retain cleanup ownership; a second signal can still force exit. A 35-second observation interval logs pending shutdown while retaining the same operations; it is not a forced-exit deadline. Signal handling remains active while the application runtime drains started blocking operations, including program-store reads whose HTTP waiters have timed out. Logging drains after that runtime finishes. A second explicit signal permits a nonzero forced exit during either drain. Individual HTTP deadlines or disconnects still leave transaction outcomes uncertain; clients reconcile their durable operation identities.
...
Orchestrator logs use a bounded nonblocking writer with 64 queued records, at most 256 KiB each and a five-second delivery deadline. Optional lost records are counted without changing the final process result; output-destination failures still make it nonzero. A paused pipe reader does not block recovery or signals. Writer ownership and signal handling persist through output drain; an uninterruptible filesystem write can still require the explicit second-signal force path.
