...
An accepted settlement stores the immutable command and receipt with its lifecycle changes. A matching accepted receipt is replayed **before** checking current lease liveness, even when a later attempt is already active. Changed report content or operation identity conflicts; new stale reports cannot mutate scheduling. The future adapter must retain a bounded diagnostic rejection record, or an attributable operation log, for rejected late reports without replacing the accepted outcome.
