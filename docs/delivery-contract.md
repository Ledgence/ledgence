...
A retry preserves run ID, task ID, input, and descriptor/digest. It allocates a new attempt ID/number, lease identity/generation, and invocation CloudEvent ID. Redelivery of one assignment keeps its original event, including its trace context. Origin and worker processing trace contexts are distinct; an accepted report can carry the latter without rewriting the event. The optional [OpenTelemetry adapter](observability.md) activates tracing and exports these contexts through OTLP/HTTP without changing durable delivery semantics. [CloudEvents tracing extension](https://github.com/cloudevents/spec/blob/main/cloudevents/extensions/distributed-tracing.md).
...
The driver requests up to 20 seconds of waiting by default. `AcquireOptions` separates the wait preference and local exchange deadline from durable `AcquireCommand` identity. A Pending storage probe rolls back its cursor placeholder and releases its transaction before waiting. At the fixed wait deadline, a final atomic probe either claims work, replays the latest cursor, or commits Empty. Errors never fabricate Empty. See [acquisition waits](acquisition-waits.md) for queue coordination, replay notifications, and bounds.

Cursors for a live session must not be deleted/reset. A cursor's referenced task/attempt must remain resolvable: retain their complete referenced snapshots after ordinary terminal retention expires until the cursor advances or its session expires. Otherwise an idle consumer could be stranded when it next tries to advance. Once a session expires, its cursor records may be removed after outstanding ownership is reconciled; an unknown session still rejects old commands. This avoids an indefinitely growing receipt history for idle polls. Renewals similarly retain one ordered command, not a history row per heartbeat.
...
| Maximum acquisition wait / control request deadline | 20 seconds / 30 seconds, including admission and transfer |
...
The minimum public execution budget exceeds the control request deadline and safety margin. Every assignment requires a fresh Dispatch renewal before starting work. Its request-start time and fresh remaining budget establish the first local execution bound; the preceding acquisition wait is excluded. These initial values establish bounded behavior, not throughput promises. Execution duration, leases, cleanup, and byte limits are distinct from the sole concurrency parameter. Existing local-worker timeouts retain their previous behavior.

The PostgreSQL adapter provides explicit [bounded retention maintenance](retention.md), scoped to a tenant and namespace, with a minimum 90-day terminal window. It preserves active execution state, current cursor snapshots and unfinished completion obligations. Retired IDs return NotFound while physical collection proceeds; submission-key reuse can create a new task after identity deletion completes. Such submission deduplication is not permanent business idempotency. Per-consumer cursor compaction never turns an old sequence into a new assignment, even after session record removal.
